...
class StyleImpl implements Style {
	...
	public static final StyleImpl EMPTY =
			new StyleImpl(null, null, new DecorationMap(new HashMap<>()));
	...
	static StyleImpl create(Key font, TextColor color, DecorationMap decorations) {
		if (font == null && color == null && decorations.isEmpty())
			return EMPTY;
	...
	@Override
	public DecorationMap decorations() {
		return decorations;
	}

	@Override
	public Style decoration(TextDecoration decoration) {
		return decoration(decoration, decoration.activeState());
	}

	@Override
	public Style decoration(TextDecoration... decorations) {
		Map<TextDecoration, TextDecoration.State> map = new HashMap<>();
		for (TextDecoration decoration : decorations)
			if (!this.decorations.contains(decoration)
					|| this.decorations.getState(decoration) != decoration.activeState())
				map.put(decoration, decoration.activeState());
		if (map.isEmpty())
			return this;
	...
	public Style decoration(TextDecoration decoration, TextDecoration.State state) {
		if (decorations.contains(decoration) && decorations.getState(decoration) == state)
			return this;
	...
	@Override
	public TextDecoration.State decorationState(TextDecoration decoration) {
	...
	@Override
	public Style font(@NotNull Key font) {
