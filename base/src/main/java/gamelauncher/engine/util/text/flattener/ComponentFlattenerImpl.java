...
final class ComponentFlattenerImpl implements ComponentFlattener {
	static final ComponentFlattener TEXT_ONLY =
			new BuilderImpl().mapper(TextComponent.class, TextComponent::content).build();
	...
	ComponentFlattenerImpl(final Map<Class<?>, Function<?, String>> flatteners,
			final Map<Class<?>, BiConsumer<?, Consumer<Component>>> complexFlatteners,
			final @Nullable Function<Component, String> unknownHandler) {
		this.flatteners = Map.copyOf(flatteners);
		this.complexFlatteners = Map.copyOf(complexFlatteners);
	...
	private void flatten0(final @NotNull Component input, final @NotNull FlattenerListener listener,
			final int depth) {
		...
		if (input == Component.empty())
			return;
		if (depth > MAX_DEPTH) {
			throw new IllegalStateException("Exceeded maximum depth of " + MAX_DEPTH
					+ " while attempting to flatten components!");
		}
	...
	private <T extends Component> @Nullable Handler flattener(final T test) {
		final Handler flattener =
				this.propagatedFlatteners.computeIfAbsent(test.getClass(), key -> {
					// direct flatteners (just return strings)
					final @Nullable Function<Component, String> value =
							(Function<Component, String>) this.flatteners.get(key);
					if (value != null)
						return (component, listener, depth) -> listener.component(
								value.apply(component));

					for (final Map.Entry<Class<?>, Function<?, String>> entry : this.flatteners.entrySet()) {
						if (entry.getKey().isAssignableFrom(key)) {
							return (component, listener, depth) -> listener.component(
									((Function<Component, String>) entry.getValue()).apply(
											component));
						}
					}

					// complex flatteners (these provide extra components)
					final @Nullable BiConsumer<Component, Consumer<Component>> complexValue =
							(BiConsumer<Component, Consumer<Component>>) this.complexFlatteners.get(
									key);
					if (complexValue != null)
						return (component, listener, depth) -> complexValue.accept(component,
								c -> this.flatten0(c, listener, depth));

					for (final Map.Entry<Class<?>, BiConsumer<?, Consumer<Component>>> entry : this.complexFlatteners.entrySet()) {
						if (entry.getKey().isAssignableFrom(key)) {
							return (component, listener, depth) -> ((BiConsumer<Component, Consumer<Component>>) entry.getValue()).accept(
									component, c -> this.flatten0(c, listener, depth));
						}
					}
		...
		if (flattener == Handler.NONE) {
			return this.unknownHandler == null
					? null
					: (component, listener, depth) -> listener.component(
	...
	static final class BuilderImpl implements Builder {
		...
		BuilderImpl(final Map<Class<?>, Function<?, String>> flatteners,
				final Map<Class<?>, BiConsumer<?, Consumer<Component>>> complexFlatteners,
				final @Nullable Function<Component, String> unknownHandler) {
		...
		private static void testHierarchy(final Class<?> existing, final Class<?> beingRegistered) {
			if (!existing.equals(beingRegistered) && (existing.isAssignableFrom(beingRegistered)
					|| beingRegistered.isAssignableFrom(existing))) {
				throw new IllegalArgumentException(
						"Conflict detected between already registered type " + existing
								+ " and newly registered type " + beingRegistered
								+ "! Types in a component flattener must not share a common hierarchy!");
			}
		}

		public @NotNull ComponentFlattener build() {
			return new ComponentFlattenerImpl(this.flatteners, this.complexFlatteners,
					this.unknownHandler);
		}

		@Override
		public <T extends Component> ComponentFlattener.@NotNull Builder mapper(
				final @NotNull Class<T> type, final @NotNull Function<T, String> converter) {
		...
		@Override
		public <T extends Component> ComponentFlattener.@NotNull Builder complexMapper(
		...
		@Override
		public ComponentFlattener.@NotNull Builder unknownMapper(
				final @Nullable Function<Component, String> converter) {
