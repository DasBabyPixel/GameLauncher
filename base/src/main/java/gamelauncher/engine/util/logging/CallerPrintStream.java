...
public class CallerPrintStream extends PrintStream implements ConverterStream {
    ...
    public CallerPrintStream(LogLevel level, AsyncLogStream out, Logger.LoggerFlags... flags) {
        super(new LogStreamConverter(Charsets.UTF_8), true);
        ((LogStreamConverter) this.out).converterStream = this;
        this.level = level;
        this.parent = out;
        this.printLines = !Arrays.asList(flags).contains(Logger.LoggerFlags.DONT_PRINT_SOURCE);
//		this.logger = logger;
    }
