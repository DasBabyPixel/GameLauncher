...
public class Phaser {
    ...
    public Phaser(Phaser parent, int parties) {
        ...
        this.STATE.set((parties == 0) ? (long) EMPTY : ((long) phase << PHASE_SHIFT) | ((long) parties << PARTIES_SHIFT) | ((long) parties));
    }
    ...
    public int arriveAndAwaitAdvance() {
        ...
        for (; ; ) {
            ...
            if (STATE.compareAndSet(s, s -= ONE_ARRIVAL)) {
                ...
                if (!STATE.compareAndSet(s, n)) return (int) (STATE.get() >>> PHASE_SHIFT); // terminated
    ...
    public int awaitAdvanceInterruptibly(int phase) throws InterruptedException {
        ...
        if (phase < 0) return phase;
        if (p == phase) {
            Phaser.QNode node = new Phaser.QNode(this, phase, true, false, 0L);
    ...
    public int awaitAdvanceInterruptibly(int phase, long timeout, TimeUnit unit) throws InterruptedException, TimeoutException {
        ...
        if (phase < 0) return phase;
        if (p == phase) {
            Phaser.QNode node = new Phaser.QNode(this, phase, true, true, nanos);
            p = root.internalAwaitAdvance(phase, node);
            if (node.wasInterrupted) throw new InterruptedException();
    ...
    public void forceTermination() {
        ...
        while ((s = root.STATE.get()) >= 0) {
            if (root.STATE.compareAndSet(s, s | TERMINATION_BIT)) {
                // signal all threads
    ...
    private void releaseWaiters(int phase) {
        Phaser.QNode q;   // first element of queue
        Thread t;  // its thread
        AtomicReference<Phaser.QNode> head = (phase & 1) == 0 ? evenQ : oddQ;
        while ((q = head.get()) != null && q.phase != (int) (root.STATE.get() >>> PHASE_SHIFT)) {
     ...
     */
    private int internalAwaitAdvance(int phase, Phaser.QNode node) {
        // assert root == this;
        ...
        while ((p = (int) ((s = STATE.get()) >>> PHASE_SHIFT)) == phase) {
            if (node == null) {           // spinning in noninterruptible mode
                ...
                if (interrupted || --spins < 0) { // need node to record intr
                    node = new Phaser.QNode(this, phase, false, false, 0L);
                ...
                AtomicReference<Phaser.QNode> head = (phase & 1) == 0 ? evenQ : oddQ;
                Phaser.QNode q = node.next = head.get();
                if ((q == null || q.phase == phase) && (int) (STATE.get() >>> PHASE_SHIFT) == phase) // avoid stale enq
