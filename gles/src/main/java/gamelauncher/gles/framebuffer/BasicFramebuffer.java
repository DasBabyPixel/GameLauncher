...
public class BasicFramebuffer extends GLESFramebuffer {
    ...
    public BasicFramebuffer(GLES gles, int width, int height)
        ...
        this.colorTexture = gles.textureManager()
                .createTexture((ExecutorThread) Threads.currentThread());
        Threads.waitFor(
                this.colorTexture.allocate(this.width().intValue(), this.height().intValue()));
        c.glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                GL_TEXTURE_2D, this.colorTexture.getTextureId(), 0);
        this.depthStencilRenderbuffer =
                new Renderbuffer(GL_DEPTH24_STENCIL8, this.width().intValue(),
                        this.height().intValue());
        c.glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT,
                GL_RENDERBUFFER, this.depthStencilRenderbuffer.getId());
        this.checkComplete();
        this.unbind();
    }
    ...
    public void resize(int width, int height) throws GameException {
        ...
        this.bind();
        StateRegistry.currentGl().glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                GL_TEXTURE_2D, this.colorTexture.getTextureId(), 0);
        this.unbind();
        this.checkComplete();
