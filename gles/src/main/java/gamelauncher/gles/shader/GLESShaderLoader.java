...
public class GLESShaderLoader implements ShaderLoader {
	...
	private static void loadUniforms(GLESShaderProgram p, JsonObject uniforms,
	...
	private static void loadUniform(GLESShaderProgram program, Collection<Struct> structs,
		...
		if (!ouniforms.has(uniform)) {
			if (required) {
				GLESShaderLoader.logger.errorf(
		...
		Uniform u = loadUniform(program, uniform,
				GLESShaderLoader.getStruct(structs, ouniforms.get(uniform).getAsString()));
	...
	private static Collection<Struct> loadStructs(JsonArray astructs) throws GameException {
		...
		for (JsonElement estruct : astructs) {
			...
			for (JsonElement evariable : ovariables) {
				...
				custom.getVariables().put(vname, GLESShaderLoader.getStruct(structs, vtype));
			}
			structs.add(custom);
		}
		return structs;
