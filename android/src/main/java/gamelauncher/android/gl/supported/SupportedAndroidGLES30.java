...
public class SupportedAndroidGLES30 extends SupportedAndroidGLES20 implements gamelauncher.gles.gl.GLES30 {
    ...
    @RequiresApi(api = Build.VERSION_CODES.N) @Override public void glGetTransformFeedbackVarying(int program, int index, int bufsize, IntBuffer length, IntBuffer size, IntBuffer type, ByteBuffer name) {
        GLES30.glGetTransformFeedbackVarying(program, index, bufsize, length, size, type, name);
    }
    ...
    @RequiresApi(api = Build.VERSION_CODES.N) @Override public void glReadPixels(int x, int y, int width, int height, int format, int type, int offset) {
