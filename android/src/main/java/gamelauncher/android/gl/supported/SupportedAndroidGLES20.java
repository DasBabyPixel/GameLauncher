...
public class SupportedAndroidGLES20 implements gamelauncher.gles.gl.GLES20 {
    ...
    @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN_MR1) @Override public String glGetActiveAttrib(int program, int index, int[] size, int sizeOffset, int[] type, int typeOffset) {
        return GLES20.glGetActiveAttrib(program, index, size, sizeOffset, type, typeOffset);
    }

    @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN_MR1) @Override public String glGetActiveAttrib(int program, int index, IntBuffer size, IntBuffer type) {
