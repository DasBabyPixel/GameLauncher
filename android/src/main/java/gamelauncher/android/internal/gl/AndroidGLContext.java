...
public class AndroidGLContext extends AbstractGameResource implements GLContext {
    ...
    AndroidGLContext create(@Nullable AndroidGLContext context) throws GameException {
        ...
        AndroidGLLoader glLoader = launcher.serviceProvider().service(AndroidGLLoader.class);
        int[] ctxAttrib = {EGL_CONTEXT_CLIENT_VERSION, glLoader.hasGLES30() ? 3 : 2, egl.EGL_NONE};
        EGLContext ctx = egl.eglCreateContext(dpy, config, context == null ? egl.EGL_NO_CONTEXT : context.context, ctxAttrib);
        this.display = dpy;
        this.surface = surf;
        this.context = ctx;
        this.gl = launcher.serviceProvider().service(AndroidGLLoader.ANDROID_GL_LOADER).gles();
