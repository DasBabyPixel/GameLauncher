...
public class Labyrinth extends Plugin {
    ...
    @Override public void onEnable() throws GameException {
        this.launcher().gameRegistry().register(new Game(this.gkey) {
                ...
                this.launcher().guiManager().openGui(framebuffer, new ParentableAbstractGui(this.launcher()) {

                    {
                        ButtonGui g = new ButtonGui(this.launcher()) {
                            ...
                            @Override protected void buttonPressed(MouseButtonKeybindEvent e) throws GameException {
                                this.launcher().guiManager().openGui(this.framebuffer, new ParentableAbstractGui(this.launcher()) {

                                    {
                                        ColorGui g = this.launcher().guiManager().createGui(ColorGui.class);
                                        g.color().set(1, 0, 1, 0.5F);
                                        g.widthProperty().bind(this.widthProperty());
                                        g.heightProperty().bind(this.heightProperty());
                        ...
                        g.xProperty().bind(this.xProperty().add(g.widthProperty().divide(2)));
                        g.yProperty().bind(this.yProperty().add(g.heightProperty().divide(2)));
                        g.text().value(Component.text("labyrinth game lol test"));
                        this.GUIs.add(g);
                    }
            ...
            @Override protected void close0() throws GameException {
            }
